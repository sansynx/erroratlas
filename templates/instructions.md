...
After a fix works, call `erroratlas.publish_resolution` with the root cause, failed attempts, final fix, verification command, affected framework, and risk level. Published resolutions default to `public`; the first submission is a non-searchable candidate and ErrorAtlas promotes it only after an independent confirmation.
